...
### 1-5: AI 時代のマインドセット（ストックから）

`docs/lab-mindset.md` を Read して全ストックを把握する。**追加・編集はしない**（中川さんが手動で育てる）。

ファイル全文を取得した上で:
1. 当日のニュース（Step 1-4）と照らし、**「今日のニュースに刺さる視点」を 3〜5 件** 抽出
2. もし当日のニュースから新しく思いついた視点があれば、`(候補・追加検討)` ラベル付きで 1〜2 件提案（中川さんが採用すれば後で `docs/lab-mindset.md` に手動追記）

抽出時の判断:
- ストックの番号と本文をそのまま引用（中川さんの言い回しを保持）
- 「これは今日の◯◯のニュースに対応」という 1 行注釈を付ける
- タグ（`#個人` `#1人会社` `#食品` `#電力` 等）が当日テーマと噛み合うものを優先

### 1-6: 自分発信フックの素材（Neon から）
...
7. **🧭 AI 時代のマインドセット** — 3〜5 件。Step 1-5 で抽出したストック視点をそのまま列挙（番号 + 本文）。各項目に「→ 今日の◯◯のニュースに合う」など 1 行注釈を付ける
   - 当日生成した `(候補・追加検討)` がある場合はリスト末尾に分けて並べる
   - ストックは `docs/lab-mindset.md`。そのまま引用、改変しない
8. **🎯 自分発信フック** — 3〜6 件。Step 1-6 のデータをそのまま転記せず、「中川さんが自分の言葉で書ける角度」に整理して並べる
...
9. **📰 業界誌の小ネタ（大手ニュースに埋もれがち）** — 10〜20 件。日経 xtech / itmedia monoist / publickey などのニッチ記事を中心に。「大手メディアでは扱われないが現場で効く動き」を拾う
...
## X Phoenix Transformer 対応（2026-05-15 以降）

このネタ集は **1 日 3 枠ルール** 前提で素材化する。中川さんは下記の確定ルーティンでこの素材から投稿を組み立てる:

- **朝**: 挨拶兼デモ宣伝（スクショ付き）→ `docs/lab-demo-post.md` のパターン A/B/C から 1 つ選ぶ
- **夕方**: ねぎらい挨拶（リアル写真）→ ねぎらい寄り、記念日/雑学/名言ネタは使わない
- **夜**: マインドセット深掘り 1 視点（メディア推奨）→ Step 2 カテゴリ 7「AI 時代のマインドセット」の中から 1 つ深掘り

そのため、ネタは「中川さんが上記 3 枠に組み込みやすい粒度」で並べる。1 日 4 投稿前提のボリュームは **作らない**（旧 7 本枠運用は 2026-05-15 廃止）。

- カテゴリ 4「ニュース」「経済」「日本独自」「業界誌の小ネタ」はネタ自体は集めるが、これらは **他人投稿への contrarian 引用 RT 用の弾** として並べる位置づけ
- 中川さんが自分のオリジナル投稿に組み込むのは、原則 **「自分発信フック」「マインドセット」「デモ宣伝」** の 3 系統のみ
