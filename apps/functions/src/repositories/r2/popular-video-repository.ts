...
export class PopularVideoRepository extends R2BaseRepository {
  async saveVideos(
    region: string,
    category: string,
    ...
    try {
      ...
      return await this.insert(this.schemas.POPULAR_VIDEOS.prefix, rows);
    } catch (error) {
      logger.error("Error saving popular videos to R2", { error });
      throw error;
    }
  }
}
