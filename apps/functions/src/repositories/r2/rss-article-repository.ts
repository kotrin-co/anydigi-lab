...
export class RssArticleRepository extends R2BaseRepository {
  async saveArticles(articles: ArticleData[]) {
    try {
      ...
      return await this.insert(this.schemas.RSS_ARTICLES.prefix, rows);
    } catch (error) {
      logger.error("Error saving RSS articles to R2", { error });
      throw error;
    }
  }
}
