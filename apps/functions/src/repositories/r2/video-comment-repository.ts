...
export class VideoCommentRepository extends R2BaseRepository {
  async saveComments(comments: youtube_v3.Schema$CommentThread[]) {
    try {
      ...
      return await this.insert(this.schemas.YOUTUBE_COMMENTS.prefix, rows);
    } catch (error) {
      logger.error("Error saving comments to R2", { error });
      throw error;
    }
  }
}
